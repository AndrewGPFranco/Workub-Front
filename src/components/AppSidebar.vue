  ...
  <header class="site-header">
    <nav class="navbar" :aria-label="t('demands.mainNav')">
      ...
      <div class="navbar-center">
        ...
        <RouterLink v-if="canAccess('DAILY')" :class="['nav-link', {active: route.name === 'Daily'}]"
                    :to="{name: 'Daily'}">
          <i class="pi pi-calendar-clock"/><span>{{ t('daily.nav') }}</span>
        </RouterLink>
        <RouterLink v-if="canAccess('FEEDBACK')" :class="['nav-link', {active: route.name === 'Feedback'}]"
                    :to="{name: 'Feedback'}">
          <i class="pi pi-comments"/><span>{{ t('feedback.nav') }}</span>
        </RouterLink>
        <RouterLink v-if="canAccess('SUBDOMAINS')" :class="['nav-link', {active: route.name === 'Subdomain Register'}]"
                    :to="{name: 'Subdomain Register'}">
          <i class="pi pi-sitemap"/><span>{{ t('subdomain.nav') }}</span>
        </RouterLink>
        <RouterLink v-if="canAccess('NOTES')" :class="['nav-link', {active: route.name === 'Notes'}]"
                    :to="{name: 'Notes'}">
          <i class="pi pi-pencil"/><span>{{ t('notes.nav') }}</span>
        </RouterLink>
      </div>
...
.brand, .navbar-center, .navbar-actions, .profile-copy {
  display: flex;
  align-items: center;
}
